...
async function fixFinal3CardsClean() {
  ...
  console.log(`Processing final ${missingCards.length} cards with full attachment compression...`);
  ...
  for (let i = 0; i < missingCards.length; i++) {
    ...
    if (c.attachments_json) {
      try {
        let atts = typeof c.attachments_json === 'string' ? JSON.parse(c.attachments_json) : c.attachments_json;
        if (Array.isArray(atts)) {
          for (let att of atts) {
            if (att.url) att.url = await compressImageToSmall(att.url);
          }
    ...
    const fileName = `fix_clean_card_${i+1}.sql`;
...
fixFinal3CardsClean().catch(console.error);
