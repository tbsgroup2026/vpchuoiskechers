...
async function fixRemainingCards() {
  ...
  for (let i = 0; i < missingCards.length; i++) {
    ...
    fs.writeFileSync('single_card_fixed.sql', sql, 'utf8');

    try {
      execSync('npx wrangler d1 execute vpchuoiskechers-db --remote --file=single_card_fixed.sql', { stdio: 'pipe' });
      console.log(`  -> Successfully inserted card ${c.id}!`);
    } catch (err) {
      console.error(`  -> Failed inserting card ${c.id}:`, err.message);
    } finally {
      if (fs.existsSync('single_card_fixed.sql')) fs.unlinkSync('single_card_fixed.sql');
    }
  }
...
fixRemainingCards().catch(console.error);
