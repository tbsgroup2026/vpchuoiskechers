import { validateScopeAuthorization, getSecurityAuditLogs } from '../web/src/lib/scopeAuth';
import { SYSTEM_USERS } from '../web/src/lib/userProfiles';
import { getEffectiveScope } from '../web/src/lib/equipmentScope';
...
const transitions = ['ALL', 'OFFICE', 'EAST', 'KIEN_GIANG'] as const;
...
const unauthorizedUser = {
  ...
  allowedScopes: ['KIEN_GIANG'] as any,
};
...
assert(Boolean(multiScopeUser && multiScopeUser.allowedScopes?.includes('OFFICE')), 'Multi-scope user has OFFICE access');
assert(Boolean(multiScopeUser && multiScopeUser.allowedScopes?.includes('EAST')), 'Multi-scope user has EAST access');
