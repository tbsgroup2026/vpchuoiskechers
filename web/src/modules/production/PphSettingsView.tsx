...
import PphShortfallCausesView from './PphShortfallCausesView';
import PphDashboardLinesView from './PphDashboardLinesView';
import PphFactoryConfigView from './PphFactoryConfigView';
import PphShoeModelsView from './PphShoeModelsView';
import PphShoePphTargetsView from './PphShoePphTargetsView';
import PphTvMediaSlotsView from './PphTvMediaSlotsView';

type PphTeam = { id: string; name: string; imageUrl?: string | null; paused?: boolean };
type PphLine = { id: string; name: string; imageUrl?: string | null; paused?: boolean; teams: PphTeam[] };
type PphArea = { id: string; name: string; imageUrl?: string | null; paused?: boolean; lines: PphLine[]; teams: PphTeam[] };
...
export default function PphSettingsView({ onClose }: { onClose: () => void }) {
  ...
  // Các màn con của Cài Đặt — gộp chọn qua 1 combobox cho gọn (thay vì mỗi cái 1 nút).
  //   'tree'    = cây Nhà máy/Xưởng/Chuyền/Tổ + mã QR (mặc định, chính là phần bên dưới)
  //   'lines'   = cấu hình Line để hiện dashboard (gán tay từng tổ vào từng line)
  //   'factory' = cấu hình hiển thị nhà máy (Target PPH %, tiêu đề, tên GĐĐH/QLCL/TH)
  //   'time'    = ràng buộc thời gian
  //   'causes'  = nhóm nguyên nhân hụt chỉ tiêu (5M1E)
  const [subView, setSubView] = useState<'tree' | 'lines' | 'factory' | 'time' | 'causes' | 'shoeModels' | 'pphTargets' | 'tvMedia'>('tree');
  // CHẾ ĐỘ DEMO — bật thì dashboard Hiệu suất tự sinh số liệu MINH HOẠ (không ghi CSDL) để trình
  // bày giao diện cho sếp. Lưu ở localStorage, dashboard tự đọc.
  const [demoOn, setDemoOn] = useState(false);
  useEffect(() => {
    try {
      setDemoOn(localStorage.getItem('pph_demo_mode') === '1');
    } catch {}
  }, []);
  function toggleDemo() {
    const next = !demoOn;
    setDemoOn(next);
    try {
      if (next) localStorage.setItem('pph_demo_mode', '1');
      else localStorage.removeItem('pph_demo_mode');
    } catch {}
  }
  ...
  // Ảnh sản phẩm gắn theo từng điểm quét (Tổ/Chuyền-lá/Xưởng-lá) — hiện thay "Mã hàng" chữ trên
  // thẻ Tổ ở dashboard. Tải thẳng lên Cloudinary (đúng cơ chế ảnh đã dùng ở các nơi khác trong
  // web này, VD ShoeLinesManager) rồi lưu lại URL qua PUT /api/pph/org/:id.
  const [uploadingImageId, setUploadingImageId] = useState<string | null>(null);
  const [imageError, setImageError] = useState<string | null>(null);
  // Tạm ngưng sản xuất (VD Chuyền hết đơn hàng) — tắt chớp đỏ/nhắc giọng đọc trên TV cho đúng điểm
  // quét đó mà KHÔNG xoá hẳn (giữ lại xem lịch sử/bật lại khi có hàng trở lại), xem entryStatus
  // "paused" ở GET /api/pph/dashboard.
  const [togglingPausedId, setTogglingPausedId] = useState<string | null>(null);

  async function handleTogglePaused(id: string, currentlyPaused: boolean) {
    setTogglingPausedId(id);
    try {
      const res = await fetch(`/api/pph/org/${id}`, {
        method: 'PUT',
      ...
      if (!result.success) {
        alert(result.error || 'Không đổi được trạng thái');
        return;
      }
      await load();
    } catch {
  ...
  async function handleUploadImage(id: string, file: File) {
    ...
    setImageError(null);
    try {
      const { secure_url } = await uploadCloudinaryFile(file, { category: 'pph_team' });
      const res = await fetch(`/api/pph/org/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
      ...
      const result = await res.json();
      if (!result.success) {
        setImageError(result.error || 'Không lưu được ảnh');
        return;
      }
      await load();
    } catch (err) {
      console.warn('Tải ảnh điểm quét thất bại:', err);
      setImageError('Không tải được ảnh lên — vui lòng thử lại.');
    } finally {
  ...
  const QrChip = ({
    ...
    const uploading = uploadingImageId === id;
    const togglingPause = togglingPausedId === id;
    return (
      <div
        className={`flex items-center gap-1 pl-2 pr-1.5 py-2 rounded-2xl border-2 ${
          paused ? 'bg-slate-100 border-slate-300 text-slate-500' : 'bg-emerald-50 border-emerald-200 text-emerald-800'
        }`}
      >
        {/* Ảnh sản phẩm — bấm vào ảnh (hoặc icon máy ảnh nếu chưa có) để tải/đổi ảnh, hiện thay
            "Mã hàng" chữ trên thẻ Tổ ở dashboard. */}
        <label
          className="relative w-9 h-9 rounded-lg overflow-hidden bg-white border border-emerald-300 flex-shrink-0 flex items-center justify-center cursor-pointer hover:brightness-95"
          title="Tải/đổi ảnh sản phẩm"
        >
          <input
            type="file"
            accept="image/*"
          ...
          />
          {uploading ? (
            <IconLoader2 size={14} className="animate-spin text-emerald-500" />
          ) : imageUrl ? (
            // eslint-disable-next-line @next/next/no-img-element
            <img src={imageUrl} alt="" className="w-full h-full object-cover" />
          ) : (
            <IconCamera size={14} className="text-emerald-400" />
          )}
        </label>
        <button type="button" onClick={() => setQrTeam({ id, name, path })} className="flex items-center gap-2 font-black text-sm flex-1 min-w-0">
          <IconQrcode size={18} className={`flex-shrink-0 ${paused ? 'text-slate-400' : 'text-emerald-500'}`} />
          <span className="truncate">{name}</span>
          {paused && <span className="shrink-0 rounded-full bg-slate-300 px-1.5 py-0.5 text-[10px] font-black text-slate-600">TẠM NGƯNG</span>}
        </button>
        {/* Tạm ngưng sản xuất — tắt chớp đỏ/nhắc giọng đọc trên TV cho điểm quét này (VD hết đơn
            hàng, chưa có gì để báo cáo) mà KHÔNG xoá hẳn như nút Xoá bên cạnh — bật lại tay khi có
            hàng trở lại. */}
        <button
          type="button"
          onClick={() => handleTogglePaused(id, !!paused)}
          disabled={togglingPause}
          className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 disabled:opacity-40 ${
            paused ? 'text-emerald-500 hover:bg-emerald-100' : 'text-slate-400 hover:bg-slate-100'
          }`}
          title={paused ? 'Bật lại sản xuất' : 'Tạm ngưng sản xuất (tắt chớp đỏ/nhắc trên TV)'}
        >
          {togglingPause ? <IconLoader2 size={14} className="animate-spin" /> : paused ? <IconPlayerPlay size={14} /> : <IconPlayerPause size={14} />}
        </button>
        <button
          type="button"
  ...
  if (subView === 'time') {
    return <PphSlotWindowsView onBack={() => setSubView('tree')} />;
  }
  if (subView === 'causes') {
    return <PphShortfallCausesView onBack={() => setSubView('tree')} />;
  }
  if (subView === 'lines') {
    return <PphDashboardLinesView onBack={() => setSubView('tree')} />;
  }
  if (subView === 'factory') {
    return <PphFactoryConfigView onBack={() => setSubView('tree')} />;
  }
  if (subView === 'shoeModels') {
    return <PphShoeModelsView onBack={() => setSubView('tree')} />;
  }
  if (subView === 'pphTargets') {
    return <PphShoePphTargetsView onBack={() => setSubView('tree')} />;
  }
  if (subView === 'tvMedia') {
    return <PphTvMediaSlotsView onBack={() => setSubView('tree')} />;
  }
  ...
  return (
    <div className="space-y-5 my-auto">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <button
            type="button"
            onClick={toggleDemo}
            title="Bật để dashboard tự sinh số liệu minh hoạ (không ghi CSDL) — trình bày giao diện cho sếp"
            className={`flex items-center gap-1.5 px-4 py-2.5 rounded-xl border text-sm font-bold ${
              demoOn ? 'bg-amber-500 border-amber-500 text-white' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'
            }`}
          >
            🧪 Data Test: {demoOn ? 'ĐANG BẬT' : 'Tắt'}
          </button>
          <label className="flex items-center gap-1.5 px-3 py-2.5 rounded-xl bg-white border border-slate-200 text-sm font-bold text-slate-600">
            <IconAdjustments size={16} className="text-slate-400 shrink-0" />
            <select
              value={subView}
              onChange={(e) => setSubView(e.target.value as typeof subView)}
              className="bg-transparent font-bold outline-none cursor-pointer"
            >
              <option value="tree">Cây Nhà máy / Mã QR</option>
              <option value="lines">Cấu hình Line (dashboard)</option>
              <option value="factory">Hiển thị Nhà máy (Target PPH…)</option>
              <option value="time">Ràng buộc thời gian</option>
              <option value="causes">Nhóm nguyên nhân</option>
              <option value="shoeModels">Ảnh theo Mã Giày</option>
              <option value="pphTargets">PPH Chuẩn theo Mã Giày (IE)</option>
              <option value="tvMedia">Chiếu ảnh/video theo khung giờ (TV)</option>
            </select>
          </label>
          <button
            type="button"
      ...
      {demoOn && (
        <div className="p-4 rounded-2xl bg-amber-50 border-2 border-amber-300 text-amber-800 text-sm font-semibold">
          🧪 <span className="font-black">Data Test đang BẬT.</span> Trang &quot;Hiệu suất Nhà máy&quot; đang hiện số liệu
          MINH HOẠ (mọi nhà máy/xưởng/tổ, có chỗ đạt / hụt / lỗi) để trình bày giao diện — <span className="font-black">KHÔNG</span> ghi
          gì vào cơ sở dữ liệu. Bấm lại nút &quot;Data Test&quot; ở trên để tắt, dashboard về dữ liệu thật ngay.
        </div>
      )}
      ...
      {imageError && (
        <div className="p-4 rounded-2xl bg-rose-50 border border-rose-200 text-rose-700 text-sm font-semibold flex items-center justify-between gap-3">
          ...
          <button type="button" onClick={() => setImageError(null)} className="text-rose-400 hover:text-rose-600 flex-shrink-0">
            <IconX size={16} />
          </button>
        </div>
      )}
              ...
              {f.areas.map((a) => {
                ...
                return (
                  <div key={a.id} className="rounded-2xl bg-slate-50 border border-slate-200 p-4 space-y-3">
                    ...
                    {isLeafArea && (
                      <QrChip id={a.id} name={a.name} path={f.name} type="AREA" imageUrl={a.imageUrl} paused={a.paused} />
                    )}
                    ...
                    {a.teams.length > 0 && (
                      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2.5">
                        {a.teams.map((t) => (
                          <QrChip key={t.id} id={t.id} name={t.name} path={`${f.name} › ${a.name}`} type="TEAM" imageUrl={t.imageUrl} paused={t.paused} />
                        ))}
                      </div>
                    )}
                    ...
                    {a.lines.length > 0 && (
                      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2.5">
                        {a.lines.map((l) =>
                          l.teams.length === 0 ? (
                            <QrChip key={l.id} id={l.id} name={l.name} path={`${f.name} › ${a.name}`} type="LINE" imageUrl={l.imageUrl} paused={l.paused} />
                          ) : (
                            <div key={l.id} className="col-span-full rounded-xl bg-white border border-slate-200 p-3 space-y-2">
                              ...
                              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                                {l.teams.map((t) => (
                                  <QrChip key={t.id} id={t.id} name={t.name} path={`${f.name} › ${a.name} › ${l.name}`} type="TEAM" imageUrl={t.imageUrl} paused={t.paused} />
                                ))}
